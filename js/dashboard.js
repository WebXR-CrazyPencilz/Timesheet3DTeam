...
function wireDashNotLoggedIn(wrap) {
  wrap.querySelector('#dashNotLoggedViewAll')?.addEventListener('click', () => {
    // Reuses the existing Attendance tab (manager.js/Client-Project.js)
    // rather than a standalone modal — same tab a Manager would open
    // manually, just one click away from here.
    if (typeof MGR_TAB !== 'undefined' && typeof renderMgrTab === 'function') {
      MGR_TAB = 'attendance';
      ...
      dashOpenNotLoggedInAll(); // fallback if manager.js isn't loaded for some reason
    }
  });
}
