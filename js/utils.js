...
function toast(type, title, msg = '', ms) {
  const dur = ms || (type==='s' ? 4000 : type==='e' ? 6000 : 3000);
  const ic = {
    ...
    e: '<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/></svg>',
    i: '<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>',
  };
  const el = document.createElement('div');
  el.className = `toast ${type}`;
  el.innerHTML = `<div class="ttico">${ic[type]||ic.i}</div><div><div class="ttit">${esc(title)}</div>${msg ? `<div class="tmsg">${esc(msg)}</div>` : ''}</div>`;
  $('tbox').appendChild(el);
